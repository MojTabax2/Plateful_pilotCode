	...
	while True:
		...
		if button_state == GPIO.LOW:
			...
			while True:
				...
				print(f"Protein={fsr_values[0]}, Carbs={fsr_values[1]}, Vegetables={fsr_values[2]}")

				# Record data locally
				if len(data_points_P) < 60 and current_time - last_db_saved >= 10:
					data_points_P.append(fsr_values[0])
					data_points_C.append(fsr_values[1])
				...
				if section_new['Vegetables'] and not section_states['Protein'] and not section_states['Carbs']:
					if fsr_values[2] >= data_points_V[-1]:  # Vegetables section not used
						tlr('Vegetables', 'light_pulse')
						if fsr_values[0] < data_points_P[-1]:
							play_audio(audio_files['vegetable'][2])
							print("protein instead of v")
						elif fsr_values[1] < data_points_C[-1]:
							tlr('carbs','light_B')
							play_audio(audio_files['vegetable'][2])
							print("carbs instead of V")

					elif fsr_values[2] < data_points_V[-1]:			
				...
				if section_new['Protein'] and not section_states['Carbs']:
					if fsr_values[0] >= data_points_P[-1]:  # protein section not used
						tlr('Protein', 'light_pulse')
						if int(fsr_values[1]/100) != 0 and fsr_values[1]/100 < data_points_C[-1]/100:
							play_audio(audio_files['protein'][3])
							print("carbs instead of p")
							
					elif fsr_values[0] < data_points_P[-1]:				
				...
				if (elapsed_time - last_checked) > 180:
					...
					if fsr_values[0] >= (sum(data_points_P) / len(data_points_P))*0.9:
						play_audio(audio_files['protein'][3])
						tlr('Protein', 'light_pulse')
				...
				# light off logic
				if elapsed_time > 360:
					if fsr_values[0] < (data_points_P[1])*0.4:  # Turn off light when FSR drops below 40%
						tlr('Protein', 'light_off')
					...
					if fsr_values[1] < (data_points_C[1])*0.4:  # Turn off light when FSR drops below 40%
						tlr('Carbs', 'light_off')
